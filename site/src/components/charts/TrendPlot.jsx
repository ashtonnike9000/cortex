...
const TREND_METRICS = [
  ...
  { key: "avg_gct_ms", path: "metrics.avg_gct_ms", label: "GCT (ms)", color: "#30d158" },
  { key: "avg_stride_len_m", path: "metrics.avg_stride_len_m", label: "Stride (m)", color: "#64d2ff" },
  { key: "avg_cadence_spm", path: "metrics.avg_cadence_spm", label: "Cadence (spm)", color: "#ffd60a" },
  { key: "avg_vgrf_peak_bw", path: "metrics.avg_vgrf_peak_bw", label: "vGRF Peak (BW)", color: "#ff453a" },
  { key: "load_total", path: "load.total", label: "Session Load", color: "#ff9f0a" },
  { key: "load_cumulative", path: "load.cumulative", label: "Cumulative Load", color: "#bf5af2" },
  { key: "distance_mi", path: "distance_mi", label: "Distance (mi)", color: "#5e5ce6" },
];
  ...
  const layout = useMemo(() => {
    ...
    active.forEach((m, i) => {
      ...
      yAxes[axisKey] = {
        ...
        gridcolor: i === 0 ? "rgba(255,255,255,0.04)" : "transparent",
    ...
    return {
      ...
      ...yAxes,
      showlegend: true,
      legend: { orientation: "h", y: -0.22, font: { size: 10, color: "#a1a1a6" }, bgcolor: "rgba(0,0,0,0)" },
      hovermode: "x unified",
      hoverlabel: {
